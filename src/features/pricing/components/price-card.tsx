    ...
    <WithSexyBorder variant={metadata.priceCardVariant} className='w-full flex-1'>
      <div className='flex w-full flex-col rounded-md border border-zinc-800 bg-black p-4 lg:p-8'>
        ...
        <div className='m-auto flex w-fit flex-1 flex-col gap-2 px-8 py-4'>
          <CheckItem
            text={
              metadata.monthlyCallLimit === Number.POSITIVE_INFINITY
                ? 'Unlimited handled calls'
                : `Up to ${metadata.monthlyCallLimit} handled calls / month`
            }
          />
          <CheckItem text={metadata.afterHours ? '24/7 after-hours coverage' : 'Business-hours only'} />
          <CheckItem text={metadata.leadSms ? 'Instant lead SMS to your phone' : 'Email lead summaries'} />
...
function CheckItem({ text }: { text: string }) {
  return (
    <div className='flex items-center gap-2'>
      <IoCheckmark className='my-auto flex-shrink-0 text-emerald-400' />
